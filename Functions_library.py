...
def h5_to_csv(h5_file_path, csv_file_path, y_source, y_field, exclude_features=None, exclude_fields=None, include_features=None, include_fields=None, batch_size=256, sample_size=100_000):
    print(f"Converting H5 files in {h5_file_path} to CSV at {csv_file_path}")
    h5_file_path = Path(h5_file_path)
    csv_file_path = Path(csv_file_path)
    h5_files = sorted(path for path in h5_file_path.iterdir() if path.is_file() and path.suffix == ".h5")
    ...
    with open(csv_file_path, 'w', newline='') as csv_file:
        # Write header
        csv_file.write(','.join(dataset.fields + [f"{y_source}_{y_field}"]) + '\n')
